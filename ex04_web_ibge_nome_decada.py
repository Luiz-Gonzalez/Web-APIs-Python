...
def pegar_nome_por_decada(nome):
    ...
    # if para verificar se api voltou sem dados e retornar uma biblioteca vazia
    if not dados_decada:
        return {}
    dict_decadas = {}
...
def main():
    ...
    # se a pessoa nao digitar nada parar
    if not nome:
        st.stop()

    dict_decadas = pegar_nome_por_decada(nome)
    #se nao voltar nada da chamda api parar e avisar
    if not dict_decadas:
        st.warning(f'Nenhum dado encontrado para o nome {nome}')
        st.stop()
    # criar a tabela com padas
